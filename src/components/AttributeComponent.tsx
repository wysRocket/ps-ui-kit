    ...
    if (this.state.opened) {
      ...
      return (
        <HGroup style={{maxWidth, ...style}}>
          ...
          <div>
            <IconButton style={{width: 24, height: 24}} onClick={this.close} size="large">
...
export class ValueViewer extends React.Component<IValueViewProps> {
  render() {
    ...
    if (
      schemaAttribute.type === SchemaDomain.AttributeType.DATE &&
      result &&
      !isNaN(parseInt(result, 10))
    ) {
      result = dateFormat(
        SchemaDomain.attributeValueToDate(result, schemaAttribute.type),
...
export class SimpleEditor extends React.Component<IEditProps> {
  ...
  onDateChanged = (newVal: Date | null) => {
    this.onChange(
      newVal ? SchemaDomain.dateToAttributeValue(newVal, this.props.schemaAttribute.type) : ""
    );
  };
  ...
  render() {
    ...
    if (schemaAttribute.type === SchemaDomain.AttributeType.DATE) {
      const value = attribute.value
        ? SchemaDomain.attributeValueToDate(attribute.value, schemaAttribute.type)
        : null;
      return (
    ...
    if (schemaAttribute.enumValues && schemaAttribute.enumValues.length) {
      const items: LabeledItem[] = schemaAttribute.enumValues.map((ev) => ({
        label: SchemaDomain.enumValueToString(ev),
        value: ev.value
      }));
    ...
    if (schemaAttribute.valueSources && schemaAttribute.valueSources.indexOf("QR") >= 0) {
      return (
        <QREditor
          ...
          textEditable={!this.props.disableQrText}
        />
      );
    }
    const multiline =
      schemaAttribute.type === SchemaDomain.AttributeType.TEXT && schemaAttribute.multiline;
